...
public:
  ...
  template<class grid_point_t>
  void xtois(const grid_point_t& x, std::vector<abs_type>& x_ids) const {
    double d_id;
    double eta_h;
    ...
    for(int k=0; k<m_dim; k++) {
      d_id = x[k]-m_first[k];
      eta_h = m_eta[k]/2.0;
      x_ids[k] = static_cast<abs_type>((d_id+eta_h )/m_eta[k]);
    }
  }
  ...
  abs_type xtoi(const grid_point_t& x) const {
    ...
    for(int k=0; k<m_dim; k++) {
      ...
      if ( d_id <= -eta_h || d_id >= m_no_grid_points[k]*m_eta[k]+eta_h ) {
        ...
        os << "is outside uniform grid, dof " << k << ", expecting: "
           << (m_first[k] - eta_h) << " <= " << x[k] << " <= "
           << (m_first[k] + m_no_grid_points[k]*m_eta[k]+eta_h);
        throw std::runtime_error(os.str().c_str());
      }
